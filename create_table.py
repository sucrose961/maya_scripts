...
def create_table(table_size_x, table_size_y, table_size_z, leg_size_x, leg_size_y, leg_size_z, x_bias, z_bias):
    ...
    if not cmds.objExists('table'):
        ...
        cmds.setAttr(leg1 + '.translate', -table_size_x*0.5*x_bias, (-leg_size_y-table_size_y)*0.5, table_size_z*0.5*y_bias, type = 'double3')
        # Duplicate a legs
        leg2, leg3 = cmds.duplicate('leg0', 'leg1')
        cmds.setAttr(leg2 + '.translate', table_size_x*0.5*x_bias, (-leg_size_y-table_size_y)*0.5, -table_size_z*0.5*y_bias, type = 'double3')
        cmds.setAttr(leg3 + '.translate', -table_size_x*0.5*x_bias, (-leg_size_y-table_size_y)*0.5, -table_size_z*0.5*y_bias, type = 'double3')
        # Grouping
